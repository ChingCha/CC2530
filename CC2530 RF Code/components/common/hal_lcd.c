...
{
	char *F_Value;								//¶r¶Í´¸º–§∆°AF_Value´¸¶Vchar´¨∫A™∫´¸º–
	F_Value = convInt32ToText(First_Value);		//F_Value´¸¶VconvInt32ToText(First_Value)
    pLcdLineBuffer[0] = First_Char;
    pLcdLineBuffer[1] = ':';
	pLcdLineBuffer[2] = *F_Value;
	
	//∞j∞È≥B≤z≥—ælpLcdLineBuffer[]
	for(int i = 3;i<=15;i++){
		pLcdLineBuffer[i] = ' ';
	}
	...
	@line:≈„•‹æl≤ƒ¥X¶Ê
	@pLcdLineBuffer[LCD_LINE_LENGTH] //∞}¶C§∏Ø¿º∆∂q≠≠®Ó(16)
	*/
	lcdWriteLine(line, pLcdLineBuffer);
}
