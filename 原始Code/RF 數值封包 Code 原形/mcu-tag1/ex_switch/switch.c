...
#define AVM_WATER     '1'					//A販賣機飲品(水)的辨識碼
#define AVM_MILK      '2'					//A販賣機飲品(牛奶)的辨識碼
...
void A_water(int A_drinkw);		//A販賣機飲品(水)的功能
void A_milk(int A_drinkm);		//A販賣機飲品(牛奶)的功能
...
int main()
{
	...
	int32 water = 5;						//A販賣機飲品(水)的數量
	int32 milk = 9;							//A販賣機飲品(牛奶)的數量
	...
	while (1)
    {
		...
		if (v == HAL_BUTTON_2){									//若v接收到BUTTON_2的訊號
			...
			halLcdDisplayWithButton(HAL_LCD_LINE_1,'W',water);	//顯示於LCD
			A_water(water);										//將引數water傳至A_water函數中的參數A_drinkw
		}
