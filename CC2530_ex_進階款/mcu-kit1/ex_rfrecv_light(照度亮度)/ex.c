...
void main(void)
{
    ...
    halMcuWaitMs(1000);
    halLcdClear();
    halLcdWriteLine(HAL_LCD_LINE_1, "*Bright Detect*");
    halLcdWriteLine(HAL_LCD_LINE_2, "POWER =    %");
    ...
    while (TRUE)
    {
        ...
        if(len>0)
        {  
                val = (pRxData[0] - '0')*100 + (pRxData[1] - '0')*10 + (pRxData[2] - '0');  
                M160_On(val);
                halLcdDisplayUint8(HAL_LCD_LINE_2, 8, HAL_LCD_RADIX_DEC, val);
        }
